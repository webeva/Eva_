...
const BuyCoinModal = dynamic(() =>
  import("../../components/BuyCoinModal.js/index.js")
);
  ...
  return (
    <>
      ...
      <img
        ...
        src={banner ? banner : "/images/banner.webp"}
      ></img>
      <div className={style.pfpContainer}>
        <img
          loading="lazy"
          alt={style.profilepfp}
          id="profilePFP"
          className={style.profilePFP}
          src={profile}
        ></img>
        <div
          id="profileName"
        ...
        >
          @{info.Username} (≈{" "}
          {Number(info.CoinPriceDeSoNanos / 1000000000).toFixed(3)} Deso)
          {info.IsVerified ? (
            <img
              width={22}
              height={22}
        ...
        <div
          id="followState"
          onClick={() => FollowEdit(userPage)}
          className={style.profileEdit}
        >
          {userPage ? "Edit" : isFollowing}
        </div>
        <div onClick={() => setOpen(!open)} className={style.buy}>
          Buy
        </div>

        <div>
          <div id="profileBio" className={style.profileBio}>
            {renderTags(info.Description)}
            <br></br>
            <br></br>
          </div>{" "}
        </div>
        ...
        >
          ...
          <p className={style.followers} style={{ marginLeft: "10px" }}>
            {following}{" "}
          </p>
          ...
          <br></br>
          <a
            href={"https://www.openprosper.com/u/" + info.Username}
            className={style.blockchain}
          >
            View on blockchain
          </a>
          <a
            href={"https://" + info.Username + ".nftz.me"}
            className={style.blockchain}
        ...
        <div className={style.Message}>
          {isLogged ? (
            <MessageInFeed
              feed={`#/profile/:${info.PublicKeyBase58Check}${media}`}
            ></MessageInFeed>
          ) : (
            <div
              style={{
            ...
            >
              To view these messages you must be logged in
            </div>
          )}
        </div>
      </div>
