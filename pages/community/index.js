...
const Createcommunity = dynamic(()=> import("../../components/CreateCommunityModal"))
...
export default function Communities() {
    ...
    async function getProfile(user){
      ...
      let communityarray = []      
      let communityValue = await  Promise.all(communities.split(",").map(async function (id){
    ...
    <>
      ...
      {value  && (
        ...
        <li onClick={() => setOpen(true)}>
                 <img className={style.bannerimg} src="/images/Create.png" alt="community"/>
                   <p >Create A New Community</p><br></br>
                   <h4>Click on this button to create a new Eva community on the DeSo blockchain.</h4>
             </li>
         {value.map(function (post){
           ...
           return(
             ...
             <li onClick={() => router.push(`/community/${post.PostHashHex}`)}>
                 <img className={style.bannerimg} src={`${post.PostExtraData.Banner}`} alt="community"/>
                   <p >{post.PostExtraData.Name}</p><br></br>
